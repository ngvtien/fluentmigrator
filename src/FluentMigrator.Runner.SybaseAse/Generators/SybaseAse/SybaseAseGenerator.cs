...
using FluentMigrator.SybaseAse;
...
{
    public class SybaseAseGenerator : GenericGenerator
    {
        public SybaseAseGenerator()
            : this(new SybaseAseQuoter())
        {
        }

        public SybaseAseGenerator(
            [NotNull] SybaseAseQuoter quoter)
        ...
        public SybaseAseGenerator(
            ...
            : this(new SybaseAseColumn(new SybaseAseTypeMap(), quoter), quoter, new EmptyDescriptionGenerator(), generatorOptions)
        {
        }

        protected SybaseAseGenerator(
        ...
        protected virtual string GetConstraintClusteringString(CreateConstraintExpression constraint)
        {
            ...
            if (!constraint.Constraint.AdditionalFeatures.TryGetValue(
                SybaseAseExtensions.ConstraintType, out indexType)) return string.Empty;

            return (indexType.Equals(SybaseAseConstraintType.Clustered)) ? " CLUSTERED" : " NONCLUSTERED";
        }
        ...
        public override string Generate(AlterDefaultConstraintExpression expression)
        {
            ...
            builder.AppendFormat("-- create alter table command to create new default constraint as string and run it" + Environment.NewLine +"ALTER TABLE {0} WITH NOCHECK ADD CONSTRAINT {3} DEFAULT({2}) FOR {1};",
                ...
                SybaseAseColumn.FormatDefaultValue(expression.DefaultValue, Quoter),
                Quoter.QuoteConstraintName(SybaseAseColumn.GetDefaultConstraintName(expression.TableName, expression.ColumnName)));

            return builder.ToString();
        }

        public override string Generate(InsertDataExpression expression) =>
            IsUsingIdentityInsert(expression)
                ? $"{string.Format(IdentityInsert, Quoter.QuoteTableName(expression.TableName, expression.SchemaName), "ON")}; {base.Generate(expression)}; {string.Format(IdentityInsert, Quoter.QuoteTableName(expression.TableName, expression.SchemaName), "OFF")}"
                : base.Generate(expression);

        private static bool IsUsingIdentityInsert(InsertDataExpression expression)
        {
            if (expression.AdditionalFeatures.TryGetValue(SybaseAseExtensions.IdentityInsert, out var feature))
            {
                return (bool)feature;
            }
        ...
        {
            return CompatibilityMode.HandleCompatibility("Sequences are not supported in SybaseAse2000");
        }
        ...
        {
            SybaseAseExtensions.IdentityInsert,
            SybaseAseExtensions.ConstraintType
        };
    }
}
